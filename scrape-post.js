// ==UserScript==
// @name         Instagram Post Scraper Enhanced FINAL + STOP + LIMIT FIX
// @namespace    http://tampermonkey.net/
// @version      4.1
// @description  IG dashboard scraper, STOP anytime, set scrape limit, safe, host filter, rate-limit aware, export HTML media
...
let state = {
    ...
    isMaximized: false,
    stopRequested: false,
    targetCount: 50 // default scrape 50
};
let ephemeralCred = {csrfToken: '', sessionId: '', rurCookie: ''};

function isIGHost(url) {
    try {
        let u = new URL(url);
        return (
...
function getCredentials(forcePrompt = false) {
    if (!ephemeralCred.csrfToken || !ephemeralCred.sessionId || forcePrompt) {
        let persisted = GM_getValue('IGSaveCreds_OPTIN', false);
        let oldCsrf = (persisted ? GM_getValue('csrfToken','') : '') || ephemeralCred.csrfToken || '';
        let oldSess = (persisted ? GM_getValue('sessionId','') : '') || ephemeralCred.sessionId || '';
        let oldRur  = (persisted ? GM_getValue('rurCookie','') : '') || ephemeralCred.rurCookie || '';
        ephemeralCred.csrfToken = prompt('Masukkan csrftoken:', oldCsrf) || '';
        ephemeralCred.sessionId = prompt('Masukkan sessionid:', oldSess) || '';
        ephemeralCred.rurCookie = prompt('Masukkan rur (opsional):', oldRur) || '';
        if (confirm('Ingin SIMPAN kredensial secara permanen di extension?\n(Tidak direkomendasikan kecuali perangkat sangat aman)')) {
            GM_setValue('csrfToken', ephemeralCred.csrfToken);
            GM_setValue('sessionId', ephemeralCred.sessionId);
            GM_setValue('rurCookie', ephemeralCred.rurCookie);
            GM_setValue('IGSaveCreds_OPTIN', true);
            alert('Kredensial disimpan permanen (dapat dihapus via tombol khusus).');
        } else {
            GM_deleteValue('csrfToken'); GM_deleteValue('sessionId'); GM_deleteValue('rurCookie');
            GM_setValue('IGSaveCreds_OPTIN', false);
            alert('Kredensial aktif hanya untuk sesi halaman saat ini.');
        }
    }
    return {...ephemeralCred};
}
function clearCredentialsStorage() {
    GM_deleteValue('csrfToken');
    GM_deleteValue('sessionId');
    GM_deleteValue('rurCookie');
    GM_setValue('IGSaveCreds_OPTIN', false);
    alert('Kredensial permanen dihapus. Untuk sesi aktif/halaman perlu klik ulang “CREDENTIALS”.');
    ephemeralCred = {csrfToken:'', sessionId:'', rurCookie:''};
}
...
async function fetchMediaBlobWithCreds(url) {
    if(!isIGHost(url)) throw new Error('Host tidak diizinkan: '+url);
...
async function fetchAsBase64(url, type='image') {
    ...
    try {
        ...
        if (blob.size > (window.IG_MAX_IMG_KB||1024)*1024) return '';
...
async function fetchVideoObjectUrl(url) {
    try {
        ...
        log(`Video fetch error for ${url}: ${e.message||e}`, "ERROR");
        return '';
    }
}
...
async function fetchWithRetry(url, options = {}, maxTries = CONFIG.MAX_RETRY, delayMs = CONFIG.DELAY_BETWEEN_REQUEST) {
    ...
    while (attempt < maxTries) {
        ...
        attempt++;
        let jitter = Math.floor(Math.random() * 1000 + 350);
...
async function scrapeInstagramPosts() {
    state.postAfterCursor=null;
    state.scrapedPostIds.clear();
    state.scrapedRows=[];
    ...
    try {
      do {
        if (warnDialog || state.stopRequested || state.scrapedRows.length >= state.targetCount) break;
        await scrapePostsPage(tableBody, username, state.postAfterCursor, progressCb);
      } while(state.isRunning && state.postAfterCursor && !warnDialog && !state.stopRequested && state.scrapedRows.length < state.targetCount);
    }
    finally {
        state.isRunning = false;
        let msg = state.stopRequested ? `STOPPED. ${state.scrapedRows.length} posts didapat!`
            : state.scrapedRows.length >= state.targetCount ? `LIMIT posts (${state.targetCount}) selesai.`
            : `ALL DONE: ${state.scrapedRows.length} posts scraped!`;
        updateStatus(msg, 'success', 1);
        log(msg, 'WARN');
    }
}

// ---------- FIX: Precise LIMIT on each batch -------------
async function scrapePostsPage(tableBody, username, afterCursor=null, progressCb=()=>{}) {
    ...
    try {
        ...
        let postsToTake = posts.filter(node => !state.scrapedPostIds.has(node.id));
        let targetLeft = state.targetCount - state.scrapedRows.length;
        postsToTake = postsToTake.slice(0, targetLeft);
        for (const node of postsToTake) {
            const postId = node.id;
            state.scrapedPostIds.add(postId);
            newPostsCount++;
        ...
        state.postAfterCursor = ((hasNextPage && state.scrapedRows.length < state.targetCount && !state.stopRequested) ? nextMaxId : null);
...
async function exportToHTML(username, isBatch=true) {
    let embedVideo = confirm('Embed VIDEO file ke HTML sebagai blob? (OK=embed, Cancel=link-only)');
    let maxImgKb = parseInt(prompt('Batas ukuran gambar in-KB untuk embed base64 (default 1024):', 1024),10) || 1024;
    window.IG_MAX_IMG_KB = maxImgKb;
    let batchSize = parseInt(state.BATCH_SIZE) || 50;
    let rows = isBatch ? state.scrapedRows.slice() : [];
    ...
    for(let i=0, batch=1; i<total; i+=batchSize, batch++){
        ...
        for (let rowItem of part) {
            for (let m of rowItem.mediaList) {
                if (m.type === "PHOTO") {
                    if (!m.datauri || m.datauri==='#skip') {
                        m.datauri = await fetchAsBase64(m.url, "image");
                        if (!m.datauri) m.datauri = '';
                    }
                } else if (m.type === "VIDEO") {
                    if (embedVideo) {
                        m.datauri = await fetchVideoObjectUrl(m.url);
                    } else {
                        m.datauri = '';
                    }
                }
...
<div class="post-preview-wrap">
    ${
        m.type=="VIDEO"
        ? (embedVideo && m.datauri
            ? `<video src="${m.datauri}" controls muted playsinline preload="none"></video>`
            : `<a class="media-link" href="${m.url}" target="_blank">[VIDEO: link]</a>`)
        : (m.datauri
            ? `<img src="${m.datauri}" alt="img">`
            : `<a class="media-link" href="${m.url}" target="_blank">[IMG: link]</a>`)
    }
...
dashboard.innerHTML = `
    ...
    <div style="display:flex;gap:8px;margin-bottom:5px;">
      <button id="stop-btn" style="flex:1;background:#FF4040;border:1px solid #D0D0D0;padding:4px;color:#fff;cursor:pointer;">STOP</button>
      <span style="margin-left:10px;font-size:13px;">Jumlah Post: <input type="number" id="target-count-input" min="1" max="5000" value="${state.targetCount}" style="width:65px;margin:0 4px;color:#000000;"></span>
    </div>
...
document.getElementById('minimize-btn').onclick=()=>{
    const box=document.getElementById('post-table-container');
    box.style.display=box.style.display==='none'?'block':'none';
    log('Table ' + (box.style.display==='none'?'minimized':'restored'), 'INFO');
};
function toggleMaximize() {
    const dashboard = document.getElementById('insta-scraper-dashboard');
    const maximizeBtn = document.getElementById('maximize-btn');
    if (!dashboard) return;
    if (state.isMaximized) {
        dashboard.style.width = 'clamp(380px,32vw,560px)';
        dashboard.style.height = '';
        dashboard.style.top = '3vh';
        dashboard.style.right = '2vw';
        dashboard.style.maxHeight = '96vh';
        dashboard.style.bottom = '';
        maximizeBtn.textContent = '□';
        state.isMaximized = false;
        log('Dashboard restored to normal size', 'INFO');
    } else {
        dashboard.style.width = '94%';
        dashboard.style.height = '94%';
        dashboard.style.top = '3%';
        dashboard.style.right = '3%';
        dashboard.style.bottom = '3%';
        dashboard.style.left = '3%';
        dashboard.style.maxHeight = '94%';
        maximizeBtn.textContent = '❐';
        state.isMaximized = true;
        log('Dashboard maximized', 'INFO');
    }
}
...
let btnHapusCred = document.createElement('button');
btnHapusCred.textContent = "HAPUS KREDENSIAL";
btnHapusCred.style = 'margin-left:4px;background:#FF4040;color:#fff;border:1px solid #D0D0D0;padding:4px;cursor:pointer;';
btnHapusCred.onclick = clearCredentialsStorage;
credentialsBtn.parentElement.appendChild(btnHapusCred);
...
document.getElementById('stop-btn').onclick = function() {
    ...
    updateStatus("STOP requested! Menunggu proses selesai...", "error");
    log("User menekan STOP. Proses akan berhenti setelah batch ini.", "WARN");
};
document.getElementById('target-count-input').onchange = function(e) {
    let v = parseInt(e.target.value);
    state.targetCount = (!isNaN(v) && v >= 1 && v <= 5000) ? v : 50;
    log(`Target jumlah post di-set ke ${state.targetCount}`);
};
})();
...
#maximize-btn:hover { background:#43a5f9!important; }
#minimize-btn:hover { background:#43a5f9!important; }
...
#post-table th:nth-child(2), #post-table td:nth-child(2) { min-width:62px;text-align:center;white-space:nowrap;padding:7px 13px;}
#post-table th:nth-child(3), #post-table td:nth-child(3) { min-width:90px;}
#post-table th:nth-child(4), #post-table td:nth-child(4) { min-width:150px; max-width:350px; white-space: normal !important; overflow: visible !important; text-overflow: unset !important; line-height: 1.4; word-break: break-word;}
